...
class UploadApi
{
    ...
    /**
     * Operation generateUrlWithHttpInfo
     *
     * Create a presigned URL for Amazon S3
     *
     ...
     */
    public function generateUrlAsync($generate_url_request, string $contentType = self::contentTypes['generateUrl'][0])
    {
        return $this->generateUrlAsyncWithHttpInfo($generate_url_request, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation generateUrlAsyncWithHttpInfo
     *
     * Create a presigned URL for Amazon S3
     *
     * @param  \DealMaker\Model\GenerateUrlRequest $generate_url_request (required)
    ...
    /**
     * Create request for operation 'generateUrl'
     *
     * @param  \DealMaker\Model\GenerateUrlRequest $generate_url_request (required)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['generateUrl'] to see the possible values for this operation
     *
     ...
     */
    public function generateUrlRequest($generate_url_request, string $contentType = self::contentTypes['generateUrl'][0])
    {

        // verify the required parameter 'generate_url_request' is set
        if ($generate_url_request === null || (is_array($generate_url_request) && count($generate_url_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $generate_url_request when calling generateUrl'
            );
        }


        $resourcePath = '/uploads/generate_url';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($generate_url_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($generate_url_request));
            } else {
                $httpBody = $generate_url_request;
            }
