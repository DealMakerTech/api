  ...
  class V1EntitiesDealsProgressPageSummary
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DealMakerAPI::V1EntitiesDealsProgressPageSummary` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DealMakerAPI::V1EntitiesDealsProgressPageSummary`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'investment_proceeds')
        self.investment_proceeds = attributes[:'investment_proceeds']
      end

      if attributes.key?(:'fees_and_adjustments')
        self.fees_and_adjustments = attributes[:'fees_and_adjustments']
      end

      if attributes.key?(:'total_proceeds')
        self.total_proceeds = attributes[:'total_proceeds']
      end

      if attributes.key?(:'processing_fees')
        self.processing_fees = attributes[:'processing_fees']
      end

      if attributes.key?(:'invoice_deductions')
      ...
      if attributes.key?(:'paid_out')
        self.paid_out = attributes[:'paid_out']
      end

      if attributes.key?(:'available_for_payout')
        self.available_for_payout = attributes[:'available_for_payout']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          investment_proceeds == o.investment_proceeds &&
          fees_and_adjustments == o.fees_and_adjustments &&
