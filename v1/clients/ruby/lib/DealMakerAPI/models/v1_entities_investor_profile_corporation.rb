  ...
  class V1EntitiesInvestorProfileCorporation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DealMakerAPI::V1EntitiesInvestorProfileCorporation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DealMakerAPI::V1EntitiesInvestorProfileCorporation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'signing_officer')
        self.signing_officer = attributes[:'signing_officer']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          signing_officer == o.signing_officer &&
    ...
    def hash
      [id, user_id, email, type, us_accredited_category, complete, corporation, signing_officer, beneficial_owners].hash
    end
